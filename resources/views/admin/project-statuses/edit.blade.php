    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Edit Project Status</h2>
        ...
        <form action="{{ route('admin.project-statuses.update', $projectStatus) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <!-- Value -->
            <div>
                <label for="value" class="block text-sm font-medium text-gray-700 mb-2">Status Value *</label>
                <input type="text" 
                       name="value" 
                       id="value" 
                       value="{{ old('value', $projectStatus->value) }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent @error('value') border-red-500 @enderror"
                       placeholder="e.g., ready, under-construction, new-launch"
                       required>
                <p class="mt-1 text-sm text-gray-500">Use lowercase with hyphens (e.g., ready, under-construction)</p>
                @error('value')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          placeholder="Brief description of this status...">{{ old('description', $projectStatus->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" 
                       name="sort_order" 
                       id="sort_order" 
                       value="{{ old('sort_order', $projectStatus->sort_order) }}"
                       min="0"
            ...
            <div class="pt-6 border-t border-gray-200">
                <div class="flex items-center">
                    ...
                    <input type="checkbox" 
                           ...
                           id="status" 
                           value="1"
                           {{ old('status', $projectStatus->status) ? 'checked' : '' }}
