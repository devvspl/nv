...
@section('title', 'Edit About Us Entry')
@section('page-title', 'Edit About Us Entry')
@section('page-description', 'Update about us content and mission statement')
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Edit About Us Entry</h2>
        ...
        <form action="{{ route('admin.about-us.update', $aboutUs) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="mission_text" 
                          ...
                          required>{{ old('mission_text', $aboutUs->mission_text) }}</textarea>
                @error('mission_text')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div id="checklist-container" class="space-y-3">
                    @php
                        $checklistItems = old('checklist_items', $aboutUs->checklist_items ?? []);
                    @endphp
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-6 border-t border-gray-200">
                <div>
                    ...
                    <input type="number" 
                           name="sort_order" 
                           id="sort_order" 
                           value="{{ old('sort_order', $aboutUs->sort_order) }}"
                           min="0"
                ...
                <div class="flex items-center">
                    ...
                    <input type="checkbox" 
                           ...
                           id="status" 
                           value="1"
                           {{ old('status', $aboutUs->status) ? 'checked' : '' }}
...
<script>
let checklistIndex = {{ count(old('checklist_items', $aboutUs->checklist_items ?? [])) }};
let statsIndex = {{ count(old('stats', $aboutUs->stats ?? [])) }};
