...
@section('page-description', 'Manage popular cities')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-heading text-zendo-navy font-semibold">Popular Cities</h2>
            <p class="text-gray-600 mt-1">Manage your popular cities for the website</p>
        </div>
        @canDo('cities.create')
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <img src="{{ $city->image_url }}" alt="{{ $city->name }}" class="w-12 h-8 object-cover rounded mr-3">
                                    <div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $city->formatted_property_count }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @canDo('cities.delete')
<form action="{{ route('admin.cities.toggle-status', $city) }}" method="POST" class="inline">
                                    ...
                                    @method('PATCH')
                                    <button type="submit" 
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors {{ $city->status ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-red-100 text-red-800 hover:bg-red-200' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1">Get started by creating your first city.</p>
                                <a href="{{ route('admin.cities.create') }}" 
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center flex-1">
                        <img src="{{ $city->image_url }}" alt="{{ $city->name }}" class="w-16 h-12 object-cover rounded mr-3">
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 mb-1">{{ $city->name }}</h3>
                            <p class="text-sm text-gray-600 mb-2">{{ $city->formatted_property_count }}</p>
                            <div class="flex items-center space-x-4 text-xs text-gray-500">
                                <span>Order: {{ $city->sort_order }}</span>
                                <span>{{ $city->created_at->format('M d, Y') }}</span>
                            </div>
                        </div>
                    </div>
                    <form action="{{ route('admin.cities.toggle-status', $city) }}" method="POST" class="inline">
...
@endCanDo
        ...
        @empty
            <div class="bg-white rounded-lg shadow-lg p-8 text-center">
                ...
                <p class="text-lg font-medium text-gray-900">No cities found</p>
                <p class="mt-1 text-gray-600">Get started by creating your first city.</p>
                <a href="{{ route('admin.cities.create') }}" 
                   class="mt-4 inline-flex items-center px-4 py-2 bg-zendo-gold text-white font-semibold rounded-lg hover:bg-opacity-90 transition-all duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
