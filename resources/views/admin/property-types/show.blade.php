        ...
        <div class="p-6 space-y-8">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg">
                <div class="flex items-center space-x-4">
                    ...
                    <span class="text-sm text-gray-600">Sort Order: {{ $propertyType->sort_order }}</span>
                </div>
                <div class="text-sm text-gray-500">
                    <p>Created: {{ $propertyType->created_at->format('M d, Y \a\t g:i A') }}</p>
                    <p>Updated: {{ $propertyType->updated_at->format('M d, Y \a\t g:i A') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 mb-2">Mapped Service Types</h3>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-2xl font-bold text-zendo-gold">{{ $propertyType->serviceTypes->count() }}</span>
                                <span class="text-sm text-gray-600">Service Types Mapped</span>
                            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0 sm:space-x-3 pt-6 border-t border-gray-200">
                <div class="flex items-center space-x-3">
                    <form action="{{ route('admin.property-types.toggle-status', $propertyType) }}" method="POST" class="inline">
                        ...
                        @method('PATCH')
                        <button type="submit" 
                                class="inline-flex items-center px-4 py-2 {{ $propertyType->status ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }} text-white font-semibold rounded-lg transition-colors">
                            {{ $propertyType->status ? 'Deactivate' : 'Activate' }}
                        </button>
                    </form>
                </div>
