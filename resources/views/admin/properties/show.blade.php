...
@section('title', $property->title . ' - ZendoIndia Admin')

@section('page-title', 'Property Details')
@section('page-description', 'View complete property information')
...
<div class="max-w-6xl space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-3xl font-heading font-bold text-zendo-navy mb-2">{{ $property->title }}</h1>
                <p class="text-gray-600">{{ $property->location->name }}, {{ $property->city->name }}</p>
            </div>
            <div class="flex flex-col space-y-2">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $property->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Property Images</h2>
            ...
            @foreach($property->images as $image)
                <div class="relative">
                    ...
                    @if($image->image_type === 'main')
                        <span class="absolute top-2 left-2 bg-zendo-gold text-white text-xs px-2 py-1 rounded">Main Image</span>
                    @endif
                </div>
            @endforeach
        </div>
    ...
    <!-- Basic Information -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Basic Information</h2>
    ...
    <!-- Pricing & Area -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Pricing & Area</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <p class="text-xl font-semibold text-gray-900">₹{{ number_format($property->price_per_sqft) }}</p>
            </div>
            @endif
            @if($property->carpet_area)
            <div>
                <p class="text-sm text-gray-600 mb-1">Carpet Area</p>
                <p class="text-base font-medium text-gray-900">{{ number_format($property->carpet_area) }} sqft</p>
            </div>
            @endif
            @if($property->built_up_area)
            <div>
                <p class="text-sm text-gray-600 mb-1">Built-up Area</p>
                <p class="text-base font-medium text-gray-900">{{ number_format($property->built_up_area) }} sqft</p>
            </div>
            @endif
            @if($property->plot_area)
            <div>
                <p class="text-sm text-gray-600 mb-1">Plot Area</p>
                <p class="text-base font-medium text-gray-900">{{ number_format($property->plot_area) }} sqft</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Specifications</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            @if($property->specifications->total_floors)
            <div>
                <p class="text-sm text-gray-600 mb-1">Total Floors</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->total_floors }}</p>
            </div>
            @endif
            @if($property->specifications->floor_number !== null)
            <div>
                <p class="text-sm text-gray-600 mb-1">Floor Number</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->floor_number }}</p>
            </div>
            @endif
            @if($property->specifications->bedrooms)
            <div>
                <p class="text-sm text-gray-600 mb-1">Bedrooms</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->bedrooms }}</p>
            </div>
            @endif
            @if($property->specifications->bathrooms)
            <div>
                <p class="text-sm text-gray-600 mb-1">Bathrooms</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->bathrooms }}</p>
            </div>
            @endif
            @if($property->specifications->balconies)
            <div>
                <p class="text-sm text-gray-600 mb-1">Balconies</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->balconies }}</p>
            </div>
            @endif
            @if($property->specifications->parking_spaces)
            <div>
                <p class="text-sm text-gray-600 mb-1">Parking Spaces</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->parking_spaces }}</p>
            </div>
            @endif
            @if($property->specifications->furnishing_status)
            <div>
                <p class="text-sm text-gray-600 mb-1">Furnishing Status</p>
                <p class="text-base font-medium text-gray-900">{{ ucwords(str_replace('_', ' ', $property->specifications->furnishing_status)) }}</p>
            </div>
            @endif
            @if($property->specifications->facing)
            <div>
                <p class="text-sm text-gray-600 mb-1">Facing</p>
                <p class="text-base font-medium text-gray-900">{{ ucwords(str_replace('_', ' ', $property->specifications->facing)) }}</p>
            </div>
            @endif
            @if($property->specifications->age_of_property)
            <div>
                <p class="text-sm text-gray-600 mb-1">Age of Property</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->age_of_property }} years</p>
            </div>
            @endif
            @if($property->specifications->possession_date)
            <div>
                <p class="text-sm text-gray-600 mb-1">Possession Date</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->possession_date->format('M d, Y') }}</p>
            </div>
            @endif
            @if($property->specifications->rera_id)
            <div>
                <p class="text-sm text-gray-600 mb-1">RERA ID</p>
                <p class="text-base font-medium text-gray-900">{{ $property->specifications->rera_id }}</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Amenities</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <!-- Location Details -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Location Details</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
            ...
            <div>
                ...
                <p class="text-base font-medium text-gray-900">{{ $property->latitude }}, {{ $property->longitude }}</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-xl font-heading font-semibold text-zendo-navy mb-4">Publishing Information</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <p class="text-sm text-gray-600 mb-1">Created At</p>
                <p class="text-base font-medium text-gray-900">{{ $property->created_at->format('M d, Y h:i A') }}</p>
            </div>
            <div>
                <p class="text-sm text-gray-600 mb-1">Last Updated</p>
                <p class="text-base font-medium text-gray-900">{{ $property->updated_at->format('M d, Y h:i A') }}</p>
            </div>
            @if($property->published_at)
            <div>
                <p class="text-sm text-gray-600 mb-1">Published At</p>
                <p class="text-base font-medium text-gray-900">{{ $property->published_at->format('M d, Y h:i A') }}</p>
            </div>
            @endif
        </div>
    </div>
