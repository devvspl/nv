    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Edit BHK</h2>
        ...
        <form action="{{ route('admin.bhks.update', $bhk) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')

            <!-- Name -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-2">BHK Name *</label>
                <input type="text" 
                       name="name" 
                       id="name" 
                       value="{{ old('name', $bhk->name) }}"
                       ...
                       placeholder="e.g., 1 BHK, 2 BHK, 3 BHK, 4+ BHK"
                       required>
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          placeholder="Brief description of this BHK configuration...">{{ old('description', $bhk->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" 
                       name="sort_order" 
                       id="sort_order" 
                       value="{{ old('sort_order', $bhk->sort_order) }}"
                       min="0"
