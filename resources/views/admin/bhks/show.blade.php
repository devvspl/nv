    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">BHK Details</h2>
        ...
        <div class="p-6 space-y-8">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg">
                <div class="flex items-center space-x-4">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $bhk->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $bhk->status ? 'Active' : 'Inactive' }}
                    </span>
                    <span class="text-sm text-gray-600">Sort Order: {{ $bhk->sort_order }}</span>
                </div>
                <div class="text-sm text-gray-500">
                    <p>Created: {{ $bhk->created_at->format('M d, Y \a\t g:i A') }}</p>
                    <p>Updated: {{ $bhk->updated_at->format('M d, Y \a\t g:i A') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 mb-2">BHK Name</h3>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            @if($bhk->description)
                                <p class="text-gray-700 leading-relaxed">{{ $bhk->description }}</p>
                            @else
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0 sm:space-x-3 pt-6 border-t border-gray-200">
                <div class="flex items-center space-x-3">
                    <form action="{{ route('admin.bhks.toggle-status', $bhk) }}" method="POST" class="inline">
                        ...
                        @method('PATCH')
                        <button type="submit" 
                                class="inline-flex items-center px-4 py-2 {{ $bhk->status ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }} text-white font-semibold rounded-lg transition-colors">
                            {{ $bhk->status ? 'Deactivate' : 'Activate' }}
                        </button>
                    </form>
                </div>
