...
@section('content')
    <div class="space-y-6">
        <div>
            <h2 class="text-2xl font-heading text-zendo-navy font-semibold">Edit Contact Information</h2>
            <p class="text-gray-600 mt-1">Update contact information details</p>
        </div>
        ...
        <form action="{{ route('admin.contact-info.update', $contactInfo) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="section_key" value="{{ old('section_key', $contactInfo->section_key) }}"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Title <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title', $contactInfo->title) }}"
                        class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-zendo-gold"
                        required>
                    @error('title')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    @if ($contactInfo->icon)
                        <div class="mb-2">
                            <img src="{{ $contactInfo->icon_url }}" alt="Current Icon" class="h-16 w-16 object-contain">
                        </div>
                    @endif
