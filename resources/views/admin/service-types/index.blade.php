            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Property Types</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    @if($serviceType->icon)
                                        <span class="text-2xl mr-3">{{ $serviceType->icon }}</span>
                                    @endif
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $serviceType->name }}</div>
                                        @if($serviceType->description)
                                            <div class="text-xs text-gray-500">{{ Str::limit($serviceType->description, 40) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600 font-mono">{{ $serviceType->slug }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {{ $serviceType->property_types_count }} mapped
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('admin.service-types.toggle-status', $serviceType) }}" method="POST" class="inline">
                                    ...
                                    @method('PATCH')
                                    <button type="submit" 
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors {{ $serviceType->status ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-red-100 text-red-800 hover:bg-red-200' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $serviceType->created_at->format('M d, Y') }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1">Get started by creating your first service type.</p>
                                <a href="{{ route('admin.service-types.create') }}" 
    ...
    <div class="md:hidden space-y-4">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center flex-1">
                        @if($serviceType->icon)
                            <span class="text-3xl mr-3">{{ $serviceType->icon }}</span>
                        @endif
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 mb-1">{{ $serviceType->name }}</h3>
                            <p class="text-sm text-gray-600 mb-2">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {{ $serviceType->property_types_count }} property types
                                </span>
                            </p>
                            <div class="flex items-center space-x-4 text-xs text-gray-500">
                                <span>Order: {{ $serviceType->sort_order }}</span>
                                <span>{{ $serviceType->created_at->format('M d, Y') }}</span>
                            </div>
                        </div>
                    </div>
                    <form action="{{ route('admin.service-types.toggle-status', $serviceType) }}" method="POST" class="inline">
        ...
        @empty
            <div class="bg-white rounded-lg shadow-lg p-8 text-center">
                ...
                <p class="text-lg font-medium text-gray-900">No service types found</p>
                <p class="mt-1 text-gray-600">Get started by creating your first service type.</p>
                <a href="{{ route('admin.service-types.create') }}" 
                   class="mt-4 inline-flex items-center px-4 py-2 bg-zendo-gold text-white font-semibold rounded-lg hover:bg-opacity-90 transition-all duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
