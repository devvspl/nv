                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <form action="{{ route('admin.faqs.toggle-status', $faq) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="text-{{ $faq->is_active ? 'red' : 'green' }}-600 hover:text-{{ $faq->is_active ? 'red' : 'green' }}-900 p-1 rounded">
                                        @if($faq->is_active)
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="border-b border-gray-200 p-4 hover:bg-gray-50">
                ...
                <div class="flex items-center justify-end space-x-3">
                    ...
                    <form action="{{ route('admin.faqs.toggle-status', $faq) }}" method="POST" class="inline-block">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="inline-flex items-center px-3 py-1 border border-{{ $faq->is_active ? 'red' : 'green' }}-300 rounded-md text-xs font-medium text-{{ $faq->is_active ? 'red' : 'green' }}-700 bg-white hover:bg-{{ $faq->is_active ? 'red' : 'green' }}-50">
                            @if($faq->is_active)
                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636m12.728 12.728L18.364 5.636M5.636 18.364l12.728-12.728"></path>
                                </svg>
