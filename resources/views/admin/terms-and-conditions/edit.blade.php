    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Terms & Conditions Page</h2>
        ...
        <form action="{{ route('admin.terms-and-conditions.update') }}" method="POST" class="p-6 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $terms->title) }}" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent">
                    <p class="mt-1 text-sm text-gray-500">This will be displayed as the main heading on the terms page.</p>
                </div>
            </div>

            <!-- Terms Content -->
            <div class="">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Terms & Conditions Content</h3>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Content <span class="text-red-500">*</span></label>
                    <textarea name="content" id="content" rows="5" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent">{{ old('content', $terms->content) }}</textarea>
                    <p class="mt-1 text-sm text-gray-500">Use the rich text editor to format your terms &amp; conditions content.</p>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="effective_date" 
                            value="{{ old('effective_date', $terms->effective_date ? $terms->effective_date->format('Y-m-d') : '') }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent">
                    ...
                    <div>
                        ...
                        <input type="date" name="last_updated" 
                            value="{{ old('last_updated', $terms->last_updated ? $terms->last_updated->format('Y-m-d') : date('Y-m-d')) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent">
