    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Edit Category</h2>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="p-6 space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="mb-3 p-4 border border-blue-200 rounded-lg bg-blue-50">
                        ...
                        <div class="flex items-center space-x-3">
                            <img src="{{ asset('storage/' . $category->icon_upload) }}" alt="Current Icon" class="w-16 h-16 object-contain border border-gray-200 rounded-lg p-2 bg-white">
                            <div class="flex-1">
                                <p class="text-xs text-gray-600">{{ basename($category->icon_upload) }}</p>
                    ...
                    <div class="mb-3 p-4 border border-gray-200 rounded-lg bg-gray-50">
                        <label class="block text-xs font-medium text-gray-600 mb-2">
                            {{ $category->icon_upload ? 'Upload New Icon' : 'Upload Icon File' }}
                        </label>
                        <input type="file" 
                               name="icon_upload" 
                               id="icon_upload"
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="flex-1">
                                <input type="text" 
                                       ...
                                       placeholder="e.g., living-house.svg">
                            </div>
                            @if($category->icon && !$category->icon_upload)
                                <div class="flex-shrink-0">
                                    <img src="{{ $category->icon_url }}" alt="Current Icon" class="w-12 h-12 border border-gray-200 rounded-lg p-2">
                                </div>
                            @endif
                        </div>
                        <p class="mt-1 text-xs text-gray-500">Icon filename or path (e.g., living-house.svg)</p>
                        @error('icon')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent @error('link') border-red-500 @enderror"
                           placeholder="/properties/living-house">
                    <p class="mt-1 text-sm text-gray-500">URL where this category should link to</p>
                    @error('link')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
