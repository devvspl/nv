...
@section('page-description', 'Manage property categories')
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    @if($category->icon)
                                        <img src="{{ $category->icon_url }}" alt="{{ $category->name }} Icon" class="w-8 h-8 mr-3">
                                    @else
                                        <div class="w-8 h-8 bg-zendo-gold rounded-full mr-3"></div>
                                    @endif
                                    <div>
...
<form action="{{ route('admin.categories.toggle-status', $category) }}" method="POST" class="inline">
                                    ...
                                    @method('PATCH')
                                    <button type="submit" 
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors {{ $category->status ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-red-100 text-red-800 hover:bg-red-200' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1">Get started by creating your first category.</p>
                                <a href="{{ route('admin.categories.create') }}" 
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center flex-1">
                        @if($category->icon)
                            <img src="{{ $category->icon_url }}" alt="{{ $category->name }} Icon" class="w-10 h-10 mr-3">
                        @else
                            <div class="w-10 h-10 bg-zendo-gold rounded-full mr-3"></div>
                        @endif
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 mb-1">{{ $category->name }}</h3>
                            <p class="text-sm text-gray-600 mb-2">{{ Str::limit($category->description, 80) }}</p>
...
@endCanDo
        ...
        @empty
            <div class="bg-white rounded-lg shadow-lg p-8 text-center">
                ...
                <p class="mt-1 text-gray-600">Get started by creating your first category.</p>
                <a href="{{ route('admin.categories.create') }}" 
                   class="mt-4 inline-flex items-center px-4 py-2 bg-zendo-gold text-white font-semibold rounded-lg hover:bg-opacity-90 transition-all duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
