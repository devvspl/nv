...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-heading text-zendo-navy font-semibold">Hero Sections</h2>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                ...
                                @if($heroSection->description)
                                    <div class="text-xs text-gray-500">{{ Str::limit($heroSection->description, 50) }}</div>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col space-y-1">
                                    ...
                                    @if(!$heroSection->video_path && !$heroSection->poster_image)
                                        <span class="text-sm text-gray-400">No media</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
...
<form action="{{ route('admin.hero-sections.toggle-status', $heroSection) }}" method="POST" class="inline">
                                    ...
                                    @method('PATCH')
                                    <button type="submit" 
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors {{ $heroSection->status ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-red-100 text-red-800 hover:bg-red-200' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1">Get started by creating your first hero section.</p>
                                <a href="{{ route('admin.hero-sections.create') }}" 
        ...
        @forelse($heroSections as $heroSection)
            <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-gray-900 mb-1">{{ $heroSection->title }}</h3>
                        @if($heroSection->highlight_text)
                        ...
                        <div class="flex items-center space-x-4 text-xs text-gray-500 mt-2">
                            <span>Order: {{ $heroSection->sort_order }}</span>
...
@endCanDo
        ...
        @empty
            <div class="bg-white rounded-lg shadow-lg p-8 text-center">
                ...
                <p class="text-lg font-medium text-gray-900">No hero sections found</p>
                <p class="mt-1 text-gray-600">Get started by creating your first hero section.</p>
                <a href="{{ route('admin.hero-sections.create') }}" 
                   class="mt-4 inline-flex items-center px-4 py-2 bg-zendo-gold text-white font-semibold rounded-lg hover:bg-opacity-90 transition-all duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
