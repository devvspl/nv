...
@section('title', 'Edit Work Process Step')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-heading text-zendo-navy font-semibold">Edit Work Process Step</h2>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.work-processes.update', $workProcess) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $workProcess->title) }}"
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                          required>{{ old('description', $workProcess->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="step_number" id="step_number" value="{{ old('step_number', $workProcess->step_number) }}" min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                           required>
                    @error('step_number')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $workProcess->display_order) }}" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                           required>
                    @error('display_order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            @if($workProcess->icon)
                <div class="mb-4">
                    ...
                    <img src="{{ asset('storage/' . $workProcess->icon) }}" alt="Current icon" class="w-16 h-16 object-cover rounded">
                </div>
            @endif
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $workProcess->is_active) ? 'checked' : '' }}
